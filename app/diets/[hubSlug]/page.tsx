...
import { getStaticDietHubRoutes, resolveHubRoute } from "@/lib/content";
import { buildHubMetadata } from "@/lib/metadata";

export function generateStaticParams() {
  return getStaticDietHubRoutes();
}
...
export async function generateMetadata({
  ...
  const resolved = resolveHubRoute("diets", hubSlug);
  return resolved?.type === "hub" ? buildHubMetadata(resolved.hub) : {};
}

export default async function DietHubPage({ params }: { params: Promise<{ hubSlug: string }> }) {
  const { hubSlug } = await params;
  const resolved = resolveHubRoute("diets", hubSlug);
