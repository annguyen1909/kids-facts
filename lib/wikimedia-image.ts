...
export function isWikimediaUploadUrl(src: string) {
  return /^https:\/\/upload\.wikimedia\.org\/wikipedia\/commons(?:\/thumb)?\//.test(src);
}

export function isWikimediaCommonsPageUrl(url: string) {
  return /^https:\/\/commons\.wikimedia\.org\/wiki\/File:/i.test(url);
}

/** Decode a Commons File: page URL into the canonical file title. */
export function extractTitleFromCommonsUrl(url: string | undefined): string | null {
...
export function buildCommonsFilePageUrl(title: string) {
...
/** Best-effort title extraction from an upload.wikimedia.org URL. */
export function fileTitleFromWikimediaUploadUrl(src: string): string | null {
  try {
...
export function resolveCommonsFileTitle(input: {
  sourceUrl?: string;
  src?: string;
}): string | null {
  return (
    extractTitleFromCommonsUrl(input.sourceUrl) ??
    (input.src ? fileTitleFromWikimediaUploadUrl(input.src) : null)
  );
}
...
function licenseUrlFromName(licenseName: string) {
...
function isCanonicalLicenseUrl(url: string) {
  try {
...
/** Normalize Wikimedia license metadata into a schema-valid https URL. */
export function normalizeLicenseUrl(licenseUrl: string | undefined, licenseName: string) {
...
export function buildSyncedImageRecord(
  existing: Record<string, unknown>,
  commons: CommonsFileRecord,
  timestamp: string,
...
export async function fetchCommonsFiles(
  titles: string[],
  userAgent = "wildlifedb-wikimedia/1.0",
): Promise<Map<string, CommonsFileRecord>> {
  const files = new Map<string, CommonsFileRecord>();
  ...
  for (let index = 0; index < titles.length; index += batchSize) {
    ...
    if (index + batchSize < titles.length) {
      await sleep(1000);
    }
  }
...
export async function fetchCommonsFileByTitle(
  title: string,
  userAgent?: string,
): Promise<CommonsFileRecord | null> {
  const files = await fetchCommonsFiles([title], userAgent);
  return files.get(title) ?? null;
}
